...
globalkeys = gears.table.join(
    awful.key({ modkey, "Shift"   }, "s",      function() 
	    awful.spawn.with_shell("flameshot gui") end, {description="screenshot", group="launcher"} ),
    ...
    -- awful.key({ modkey, "o" }, "u", function () awful.client.opacity += 10 end, 
    --           {description = "decrease opacity"})

    -- max bright, decrease, increase
    awful.key({ modkey, "Shift" }, "u", function() 
            if client.focus then
                client.focus.opacity = math.min(client.focus.opacity + 0.1, 1)
            end
        end, 
    ...
    awful.key({ modkey, "Shift" }, "d", function() 
            if client.focus then
                client.focus.opacity = math.max(client.focus.opacity - 0.1, 0)
            end
        end, 
              {description = "decrease opacity", group = "client"}),

    -- awful.key({ modkey, "o" }, "m", function() 
    --         awful.client.opacity = 1
    --     end, 
    --           {description = "set opacity to 1", group = "client"}),
